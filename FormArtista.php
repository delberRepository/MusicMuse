<?php
   ...
   if ($_SERVER["REQUEST_METHOD"] == "POST") {
       ...
       if ($stmtUsuario->execute()) {
           ...
           // Ejecutar la consulta para insertar el artista
           if ($stmtArtista->execute()) {
               $_SESSION['resultadoRegistro'] = "Artista registrado con éxito.";
           } else {
               $_SESSION['resultadoRegistro'] = "Error al registrar el artista: " . $stmtArtista->error;
           }
          $_SESSION['artistaID'] = $con->insert_id; //capturo el id del artista generado

           $stmtArtista->close();
       } else {
           $_SESSION['resultadoRegistro'] = "Error al registrar el usuario: " . $stmtUsuario->error;
       }
...
<head>
    ...
    <style>
     ...
     <script>
        //esto lo que hace es activar un evento cuando el formulario es enviado
        document.addEventListener('DOMContentLoaded', (event) => {
            
            document.getElementById('username').setAttribute('value', '');
            document.getElementById('password').setAttribute('value', '');
          document.getElementById('miFormulario').onsubmit = 
