...
// Aquí de alguna forma a través de alguna variable de sesión tengo identificar al artista
$ID = $_SESSION['IDartista'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['eliminar_disco'])) {
    // Verificar si el ID del disco está presente en el formulario
    if (isset($_POST['disco_id'])) {
        // Obtener el ID del disco desde el formulario
        $id = $_POST['disco_id'];
        ...
        try {
            // Primero eliminar las canciones asociadas al disco
            $sql_canciones = "DELETE FROM canciones WHERE DiscoID = ?";
            $stmt_canciones = $conn->prepare($sql_canciones);
            $stmt_canciones->bind_param("i", $id);
            $stmt_canciones->execute();
            $stmt_canciones->close();
            
            // Luego eliminar el disco
            $sql_disco = "DELETE FROM Discos WHERE DiscoID = ?";
            $stmt_disco = $conn->prepare($sql_disco);
            $stmt_disco->bind_param("i", $id);
            $stmt_disco->execute();
            $stmt_disco->close();
           ...
           // echo "El disco y sus canciones asociadas han sido eliminados correctamente.";
        } catch (Exception $e) {
            // Revertir la transacción en caso de error
            $conn->rollback();
            echo "Error al eliminar el disco y sus canciones: " . $e->getMessage();
        }
    } else {
        echo "ID del disco no proporcionado.";
    }
}
?>
	  ...
	  <aside>
		...
		<section> 
         ...
         <!-- AQUI ES DODNDE ESTA LA INFORMACION GENERAL DEL ARTISTA-->
             ...
             <div class="container">
                 <div class="header">
                    <h1 class="letrasCabecera"><?php echo strtoupper($Artista); ?></h1>
                    <div class="atras">
                    ...
                    //count($discosID) > 0: Esta expresión compara el resultado de count($discosID) con 0. 
                    //Si el número de elementos en $discosID es mayor que 0, la expresión será verdadera (true). 
                    //Si el número de elementos es 0, la expresión será falsa (false).
                    $hayDiscos = count($discosID) > 0;
                    if (!$hayDiscos){
                        echo '<h2 style="font-size: 40px; " class="letrasCabecera">Aún no has subido nada.</h2>';
                        echo '<a href="FormularioAñadirMusica.php"><h1 style="font-size: 50px;" class="letrasCabecera">Sube Tu Música</h1></a>';
                        echo'<div class="letrasCabecera"><a href="FormularioAñadirMusica.php"><i class="bi bi-cassette-fill"></i></a></div>';
                  ...
                  $stmt->close();
                            ...
                            echo'<input type="hidden"  name="disco_id" value="'.$discoID.'">';
                            echo '<button type="submit"  name="eliminar_disco" class="borrar_boton">Borrar disco</button>';
                            echo'<form>';
